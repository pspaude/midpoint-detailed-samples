...
class ScriptedSqlUtils {
    ...
    static String buildWhereClause(Filter filter, Map sqlParams, String uidColumn, ColumnPrefixMapper mapper, String logPrefix) {
        return buildWhereClause(filter, sqlParams, uidColumn, uidColumn, mapper, logPrefix)
    }

    static String buildWhereClause(Filter filter, Map sqlParams, String uidColumn, String nameColumn, ColumnPrefixMapper mapper, Class<?> uidType, String logPrefix) {
        ScriptedSqlFilterVisitor visitor = new ScriptedSqlFilterVisitor(sqlParams, uidColumn, nameColumn, mapper, uidType, logPrefix)

        return buildWhereClause(filter, visitor,logPrefix)
    }

    static String buildWhereClause(Filter filter, FilterVisitor filterVisitor, String logPrefix) {
        if (filter == null) {
            LOG.info(logPrefix + "Returning empty where clause")
            return ""
        }
    ...
    static void executeQuery(String sqlQuery, Map sqlParams, OperationOptions options, Closure closure, Closure handler, Sql sql, String logPrefix) {
        String paging = buildPaging(options, sqlParams, logPrefix)

        if (!paging.isEmpty()) {
            sqlQuery = sqlQuery + " " + paging
        }

        LOG.info(logPrefix + "Select query ''{0}'', params ''{1}''", sqlQuery, sqlParams)
    ...
    static String buildPaging(OperationOptions options, Map sqlParams, String logPrefix) {
        if (options == null) {
            return ""
        }
        ...
        LOG.info(logPrefix + "Page size {0}, offset {1}", pageSize, pagedResultsOffset)
    ...
    static void testConnection(Sql sql, List<String> tables, String logPrefix) {
        Connection connection = sql.getConnection()

        LOG.info(logPrefix + "Using driver: {0} version: {1}",
        ...
        msg.append(logPrefix + "Failed test connection for tables (").append(failedCheckTables.size()).append("): ")
    ...
    static String[] getStringArray(Set attributes, String name) {
        final Attribute attr = AttributeUtil.find(name, attributes)

        if (attr != null && attr?.getValue() instanceof List<?>) {
            final List<String> strList = attr.getValue()
            return strList.toArray(new String[strList.size()])

        } else if (attr != null && attr?.getValue()?.getClass()?.isArray()) {
            final String[] strArray = attr.getValue()
            return strArray
    ...
    static void buildAndExecuteInsertQueryList(Sql sql, String table, List<Map<String, Object>> params, String logPrefix) {
        if (params == null) {
            return
        }

        for (Map<String, Object> param : params) {
            buildAndExecuteInsertQuery(sql, table, param, logPrefix)
        }
    }

    static List<List<Object>> buildAndExecuteInsertQuery(Sql sql, String table, Map<String, Object> params, String logPrefix) {
        ...
        LOG.info(logPrefix + "Insert query {0} with params {1}", query, params)

        def ret = sql.executeInsert(params, query)  //TODO redo this and ensure returning UID value only for later usage!

        LOG.info(logPrefix + "Insert query return {0}, {1}", ret, String.valueOf(ret[0][0]))
        return ret
    }

    static Query buildUpdateQuery(String table, Map<String, Object> params, Map<String, Object> where, String logPrefix) {
        if (params == null || params.isEmpty()) {
            return null
        }
    ...
    static void buildAndExecuteUpdateQuery(Sql sql, String table, Map<String, Object> params, Map<String, Object> where, String logPrefix) {
        Query query = buildUpdateQuery(table, params, where, logPrefix)
        if (query == null) {
            return
        }

        LOG.info(logPrefix + "Update query {0} with params {1}", query.query, query.params)

        int changedRows = sql.executeUpdate(query.params, query.query)

        LOG.ok(logPrefix + "Updated {0} rows", changedRows)
    }
    ...
    static int buildAndExecuteDeleteQuery(Sql sql, String table, Map<String, Object> where, String logPrefix) {
        String query = "delete from " + table + " where "

        List<String> params = prepareSqlNamedParameters(where, "", true)
        query += join(params, " and ")

        LOG.info(logPrefix + "Delete query {0} with params {1}", query, where)
